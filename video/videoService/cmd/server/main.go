...
import (
	...
	"time"
	"videoService/config"
	"videoService/internal/model"
	"videoService/internal/task"
...
func main() {
	...
	// 创建服务实例
	videoService := service.NewVideoServiceService()

	// 注册 gRPC 服务
	pb.RegisterVideoServiceServer(grpcSrv, videoService)
	...
	//定时从redis更新到数据库
	go task.StartVisitCountSyncTask(model.RefreshTime)
