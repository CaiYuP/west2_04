...
func (c *Config) InitDbConfig() {
	...
	mc.Master = master
	mc.Slave = slaves
	mc.MaxOpenConns = c.viper.GetInt("db.max_open_conns")
	if mc.MaxOpenConns == 0 {
		mc.MaxOpenConns = 200
	}
	mc.MaxIdleConns = c.viper.GetInt("db.max_idle_conns")
	if mc.MaxIdleConns == 0 {
		mc.MaxIdleConns = 10
	}
	mc.ConnMaxLifetime = c.viper.GetInt("db.conn_max_lifetime")
	if mc.ConnMaxLifetime == 0 {
		mc.ConnMaxLifetime = 3600
	}
	c.Dbc = mc
}
