...
func main() {
	...
	// 创建服务实例
	socialService := service.NewSocialServiceService()

	// 注册 gRPC 服务
	pb.RegisterSocialServiceServer(grpcSrv, socialService)

	// 创建 Kratos 应用
	app := newApp(logger, grpcSrv)

	// 启动服务
	logger.Log(kratoslog.LevelInfo, "msg", fmt.Sprintf("社交服务启动在 %s", grpcAddr))
