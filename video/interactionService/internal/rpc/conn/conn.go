...
func InitClients(cfg *ServiceConfig) (*ClientManager, error) {
	...
	// 初始化社交服务客户端（使用服务发现，支持重试）
	socialConn, err := createConnectionWithDiscoveryWithRetry(cfg.SocialServiceName, 3, 2*time.Second)
	if err != nil {
		return nil, fmt.Errorf("初始化社交服务客户端失败: %w", err)
	}
...
func GetClientManager() *ClientManager {
	...
	cfg := &ServiceConfig{
		...
		SocialServiceName: config.C.ESc.SocialService,
	}
